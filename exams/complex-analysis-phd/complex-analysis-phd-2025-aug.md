...
**8.** Let \(G\) be an open domain and let \(\{f_n\}\subset H(G)\). Also let \(f\) be a nonconstant function such that \(f_n\to f\) in \(H(G)\). Let \(a\in G\) and set \(\alpha=f(a)\). Show that there is a sequence \(\{a_n\}\subset G\) such that \(a_n\to a\) and \(f_n(a_n)=\alpha\) for all \(n\) sufficiently large.
