...
**3.** Evaluate \(\displaystyle \int_0^\infty \frac{\cos x}{(1+x^4)^2}\,dx\) using contour integration.

**4.** Show that \(\displaystyle f(s)=\sum_{n=1}^\infty \frac{(-1)^{n-1}}{n^s}\) is analytic in \(\operatorname{Re}s=\sigma>1\). Prove that \(f\) can be analytically continued to all of \(\mathbb{C}\) as an entire function. (Hint: Write \(f(s)\) in terms of \(\zeta(s)\) in \(\sigma>1\). Use properties of \(\zeta(s)\) to show that \(f\) can be made entire.)

**5.** Let \(\displaystyle f(z)=\sum_{n=1}^\infty \frac{z^n}{n^2}\). What is the radius of convergence \(R\) of this series? Is the series convergent at all points \(|z|=R\)? At what points of \(|z|=R\) is \(f\) analytic? (Hint: Think of \(f\) as the primitive of \(-\log(1-z)/z\).)
...
**9.** Suppose that \(f\) is a conformal map of unit disk onto a square with center at \(0\) and \(f(0)=0\). Prove that \(f(iz)=if(z)\). If \(\displaystyle f(z)=\sum^\infty c_nz^n\), prove that \(c_n=0\) unless \(n-1\) is a multiple of \(4\). (Hint: Show that \(if(z)\) is also a conformal map of the unit disk onto the same square.)
