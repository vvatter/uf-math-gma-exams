# Numerical Analysis, first year exam, January 2026

*Do 4 (four) problems.*

**1.** Prove each of the following.
* (a) The matrix 2-norm is invariant under unitary transformation.
* (b) If \(n\times n\) matrix \(A\) is diagonal, then for any matrix \(p\)-norm \(\lVert\cdot\rVert_p\), it holds that \(\lVert A\rVert_p=\max_{1\le i\le n}|a_{ii}|\).
...
* (b) \(\lVert A\rVert_2\le\lVert A\rVert_F\le\sqrt{n}\lVert A\rVert_2\).
