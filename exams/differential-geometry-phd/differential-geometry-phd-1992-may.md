...
**5.** Explain what is meant by a Lie group \(G\), its Lie algebra \(\mathfrak g\), and the adjoint representation \(\operatorname{Ad}\) of \(G\) on \(\mathfrak g\). How does the adjoint representation of \(\operatorname{Gl}(m,\mathbb R)\) appear in terms of the natural identifications? Show that if \(G\) is connected then the kernel of the adjoint representation coincides with the centre of \(G\). It may be assumed that if \(t\in\mathbb R\), \(g\in G\), and \(\xi\in\mathfrak g\), then
\[
