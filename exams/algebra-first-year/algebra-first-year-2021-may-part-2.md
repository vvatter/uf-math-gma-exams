...
**5.** Determine the number of similarity classes of matrices \(A\in\mathrm{M}_5(\mathbb{C})\) satisfying \(A^3=0\). Give a representative for each similarity class in Jordan canonical form.
...
* (b) Give an example of a field extension \(L/K\) of degree \(4\), a polynomial \(f(X)\in K[X]\) of degree \(5\), and a root \(\alpha\) of \(f(X)\) such that \(\alpha\in L\) but \(\alpha\notin K\).
