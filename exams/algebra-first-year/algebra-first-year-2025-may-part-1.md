...
* (a) Find the order of the group \(\operatorname{GL}(4,\mathbb{Z}/3\mathbb{Z})\).
* (b) Show that there exists a nonabelian group of order \(2025=3^4\cdot 5^2\) in which the Sylow \(3\)-subgroup is abelian.

**5.** Let \(K\) and \(H\) be groups, with \(K\) abelian. Consider a homomorphism \(\phi:K\to\operatorname{Aut}(H)\) and the associated semidirect product \(G=H\rtimes_\phi K\).
* (a) Prove that when \(K\) is identified with the subgroup \(\{(1,k)\mid k\in K\}\) of \(G\), \(\operatorname{Ker}(\phi)\) is contained in the center of \(G\).
