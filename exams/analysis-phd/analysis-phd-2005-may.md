...
**1.** Let  be a positive Radon measure on a locally compact Hausdorff space \(T\). Let \(f_n:T\to\mathbb R\) be \(\)-measurable for all \(n\in\mathbb N\). Prove that \(\sup\{f_n\mid n\in\mathbb N\}\) is \(\)-measurable.
...
**7.** Let  be a \(\sigma\)-finite positive Radon measure on a locally compact Hausdorff space \(T\). Let \(\{f_n\}_{n\in\mathbb N}\subset L^p(T,)\) for some \(p\in[1,\infty)\). Suppose that \(\lim_{n\to\infty}f_n(t)=f(t)\) -a.e., that \(f\in L^p(T,)\), and that \(\lim_{n\to\infty}\int |f_n|^p\,d=\int |f|^p\,d\). Prove that \(\lim_{n\to\infty}\int |f_n-f|^p\,d=0\). Hint: Bring the Lebesgue Dominated Convergence Theorem and the \(\sigma\)-finiteness of the measure into play.
