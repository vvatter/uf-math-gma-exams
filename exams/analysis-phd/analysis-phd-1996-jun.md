...
**4.** Let \(K_n\subset[0,1]\) be a Cantor set of Lebesgue measure larger than \(1-\frac{1}{n}\). Let \(V:=\bigcup_{n=1}^{\infty}K_n\). Prove that, for any null set \(N\), the set \(V\setminus N\) is not a \(G_\delta\) set. [Hint: Baire Category Theorem]
...
**8.** A distribution \(T\) is called harmonic if \(\Delta T=0\), where \(\Delta=\sum_{i=1}^n\frac{\partial^2}{\partial x_i^2}\). Show that if \(T\) is a harmonic tempered distribution, then \(T\) is a polynomial. [Hint: a distribution with support \(\{0\}\) is a finite sum of multiples of Dirac's \(\delta\) and its derivatives.]
