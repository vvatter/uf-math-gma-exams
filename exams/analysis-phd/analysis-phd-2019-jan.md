...
*Do three from Part A (problems 1–4) and three from Part B (problems 5–8). Write solutions in a neat and logical fashion, giving complete reasons for all steps.*
...
**6.** Let \((\varphi_n)\) be a sequence from \(L^1(\mathbb R)\) with the following properties. Show, if \(f\in L^1(\mathbb R)\), then \(\varphi_n\star f\) converges to \(f\) in \(L^1(\mathbb R)\).
