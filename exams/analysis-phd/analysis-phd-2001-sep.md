...
**2.** Prove that if \(f:X\to\overline{\mathbb R}_+\) is \(\mu\)-measurable and \(\int f\,d\mu<\infty\), then \(f<\infty\), \(\mu\)-a.e.

**3.** If \(\mu(X)<\infty\), \(f_n\in L^1(\mu)\), and \(f_n\to f\) uniformly, then \(f\in L^1(\mu)\) and \(f_n\to f\) in the mean.

**4.** Prove the existence of the conditional expectation \(E(f/\mathcal F)\) for functions \(f\in L^1(\mu)\), with respect to a \(\sigma\)-algebra \(\mathcal F\subset\Sigma\).

**5.** Prove that if \(f\in L^1\) and \(\int_A f\,d\mu=0\) for \(A\) in a ring \(R\) generating the \(\sigma\)-algebra \(\Sigma\), then \(f=0\), \(\mu\)-a.e.
