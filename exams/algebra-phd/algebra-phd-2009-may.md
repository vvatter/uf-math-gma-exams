...
**1.** Let \(\mathcal C\) be a category, and denote by \(\mathrm{Set}\) the category of sets.
* (i) Define what it means for a functor \(F:\mathcal C^{op}\to\mathrm{Set}\) to be representable.
* (ii) Show that if \(X,Y\) are objects of \(\mathcal C\) representing the same functor \(F:\mathcal C^{op}\to\mathrm{Set}\), then \(X\simeq Y\).
...
**4.** Let \(K\) be a field, \(L=K(\alpha)\) a simple extension of \(K\), and \(\overline K\) an algebraic closure of \(K\). Show that \(\alpha\) is separable over \(K\) if and only if the ring \(L\otimes_K\overline K\) has no nonzero nilpotent elements.
...
**11.** Let \(k\) be a field. A \(k\)-algebra \(R\) is central if the image of the structure map \(k\to R\) is in the center of \(R\).
