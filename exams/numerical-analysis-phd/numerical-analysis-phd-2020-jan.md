# Numerical Analysis, PhD exam, January 2020

*Do 4 (four) of the first 5 (1–5) and 4 (four) of the last 5 problems (6–10).*

**1.** Let \(A \in \mathbb{C}^{m\times n}\).
* (a) Determine constants \(\alpha\) and \(\beta\) such that the following inequality holds for the \(p\) and \(\infty\) norms of matrix \(A\), for integers \(p\geq 1\). Justify your answer.
  \[
...
**7.** Consider the fixed point problem \(x=f(x)\), where \(f(x)=e^{-(3+x)}\).
...
**8.** Let \(P_1\) be the space of polynomials of degree at most one. Using the norm \(\|u\|_2=\left(\int_a^b u^2\,dx\right)^{1/2}\)
...
* (b) Let \(\psi_0(h)=\psi(h)\) be the difference approximation to \(f''(a)\) found in part (a). Assume (in exact arithmetic) \(\psi(h)\to\psi(0)=f''(a)\) as \(h\to 0\), and that \(\psi(h)\) has the asymptotic expansion
  \[
