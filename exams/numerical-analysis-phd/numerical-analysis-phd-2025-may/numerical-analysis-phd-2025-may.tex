...
\renewcommand{\@maketitle}{%
  ...
  \vskip -0.5em%
  \tagstructbegin{tag=H1,title={Numerical Analysis PhD Exam, May 2025}}%
  \tagpdfparaOff%
  \tagmcbegin{tag=H1}%
...
\textbf{Warning: Suspected error.} Part (c) uses \(A^{-1}\), but the standing assumption \(\lVert A\rVert_p<1\) does not imply that~\(A\) is invertible. The source reading has been retained because the intended correction is not uniquely determined.
\end{problemconcern}
