...
* (b) If \(u,v \in \mathbb{C}^{m}\) and \(A=uv^{*}\), show that \(\lVert A\rVert _2=\lVert u\rVert _2\lVert v\rVert _2\).
...
**3.** If \(A \in \mathbb{R}^{m \times n}\) with \(m \geq n\), \(\operatorname{rank}(A)=n\) and \(b \in \mathbb{R}^{m}\).
