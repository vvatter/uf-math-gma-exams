...
\hypersetup{
  pdftitle={Numerical Analysis PhD Exam, January 2026},
...
\renewcommand{\@maketitle}{%
  ...
  \vskip -0.5em%
  \tagstructbegin{tag=H1,title={Numerical Analysis PhD Exam, January 2026}}%
  \tagpdfparaOff%
  \tagmcbegin{tag=H1}%
    {\LARGE\bfseries\href{https://gma.math.ufl.edu/exams/numerical-analysis-phd/}{Numerical Analysis PhD Exam}, January 2026\par}%
  \tagmcend%
  \tagpdfparaOn%
  \tagstructend%
  \vskip 0.25em%
...
\makeatother
\title{Numerical Analysis PhD Exam, January 2026}
\author{}
\date{}
\begin{document}
\maketitle
...
\item[\textnormal{(b)}]
If \(n \times n\) matrix~\(A\) is diagonal, then for any matrix~\(p\)-norm \(\|\cdot\|_p\), it holds that \(\|A\|_p = \max_{1\leq i\leq n}|a_{ii}|\).
\end{examsubparts}
