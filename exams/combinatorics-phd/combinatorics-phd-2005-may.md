...
**1.** Prove that, for any \(n+1\) numbers chosen from the set \(\{1,2,3,\ldots,2n\}\), one divides another.
...
* (a) Prove that for a \((v,k,\lambda)\)-BIBD we have \(\lambda(v-1)=r(k-1)\).
* (b) Consider a symmetric \((v,k,\lambda)\)-BIBD and denote the order by \(n:=k-\lambda\). Using calculus to minimize, prove that \(v\geq 4n-1\).
...
**8.** If \(\lambda\) is an eigenvalue of (the adjacency matrix \(A\) of) a graph with \(n\) vertices, \(m\) edges and maximum degree \(\Delta\), then prove that
* (a) \(\lambda\leq\Delta\).
* (b) \(\lambda\leq\sqrt{\frac{2m(n-1)}{n}}\).
