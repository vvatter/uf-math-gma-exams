...
\renewcommand{\@maketitle}{%
  ...
  \vskip -0.5em%
  \tagstructbegin{tag=H1,title={Combinatorics PhD Exam, May 2022}}%
  \tagpdfparaOff%
  \tagmcbegin{tag=H1}%
...
\textbf{Warning: Suspected error.} The statement as printed is false when the unique element covered by~\(y\) is the minimal element~\(0\). A condition such as \(x\ne0\) may be missing, but the intended correction is not uniquely determined, so the source reading has been retained.
\end{problemconcern}
