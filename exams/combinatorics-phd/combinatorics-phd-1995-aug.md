...
**8.** Define a \(q\)-Hamming code \(C\) as one whose parity-check matrix \(H\) has as its columns one non-zero vector from each \(1\)-dimensional subspace of the vector space \(\mathrm{GF}(q)^r\).
