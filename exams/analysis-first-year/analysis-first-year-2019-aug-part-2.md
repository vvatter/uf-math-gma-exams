...
**1.** Let \(f:[a,b]\to\mathbb{R}\) be a Riemann integrable function and suppose that \(\int_a^b f(x)\,dx>0\). Prove that there is a nonempty open interval \(I\subset[a,b]\) and a number \(\delta>0\) such that \(f(x)>\delta\) for all \(x\in I\). Does this remain true if \(f\) is only assumed to be Lebesgue integrable? Prove, or give a counterexample.

**2.** Let \((f_n)\) be a sequence of continuously differentiable functions on \([0,1]\). Prove that if \(f_n'\to g\) uniformly on \([0,1]\), and the sequence \(f_n(0)\) converges, then there is a continuously differentiable function \(f\) such that \(f_n\to f\) uniformly on \([0,1]\) and \(f'=g\).
...
**4.** Let \((X,\mathcal{M})\) be a measurable space and \(f_n:X\to\mathbb{R}\) a sequence of measurable functions. Prove that each of the following sets is measurable:
