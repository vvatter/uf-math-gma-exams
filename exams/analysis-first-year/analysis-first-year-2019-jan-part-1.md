...
**1.** Let \((s_n)_{n=1}^{\infty}\) be a bounded sequence of real numbers. Consider the sequence \((\sigma_n)\) of averages
\[
...
**3.** Let \(X,Y\) be metric spaces and \(f:X\to Y\) a function. Prove that if \(f\) is uniformly continuous on \(X\) and \((x_n)\) is a Cauchy sequence in \(X\), then \((f(x_n))\) is a Cauchy sequence in \(Y\).

**4.** Let \(X\) be a metric space and suppose that every continuous function \(f:X\to\mathbb{Z}\) is constant. Prove that \(X\) is connected.
