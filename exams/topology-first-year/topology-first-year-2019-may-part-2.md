...
**1.** Let \(n \ge 1\). Suppose that \(f,g:X\to\mathbb{R}^n\) are continuous. Show that \(f\) and \(g\) are homotopic.

**2.** Let \(\gamma:[0,1]\to X\) with basepoint \(x_0\). Let \(\gamma^{-1}\) be defined by \(\gamma^{-1}(t)=\gamma(1-t)\). Show that \(\gamma\ast\gamma^{-1}\) is loop homotopic to the constant loop \(1:[0,1]\to X\), \(1(t)\equiv x_0\).

**3.** Show that \(\pi_1(S^1,1)=\mathbb{Z}\).

**4.** Show that the disk, \(D^2=\{z\in\mathbb{C}\mid \lVert z\rVert\le 1\}\), has the fixed point property.

**5.** Consider the matrix \(M=\begin{bmatrix}2&3\\1&1\end{bmatrix}\). This represents a group homomorphism \(M:\mathbb{Z}^2\to\mathbb{Z}^2\). Show that there is a map \(f:T^2\to T^2\) such that \(f_*=M\), where \(f_*:\pi_1(T^2)\to\pi_1(T^2)\) is the homomorphism induced by \(f\) on the fundamental group.

**6.** Suppose that \(f:S^1\to S^1\) is continuous. Show that \(f\) is homotopic to \(z^n:S^1\to S^1\) for some \(n\in\mathbb{Z}\). This is the degree of \(f\).
