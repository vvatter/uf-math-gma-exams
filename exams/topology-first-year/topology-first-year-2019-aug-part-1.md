...
**3.** Let \(X\) be a Hausdorff space. Show that if \(A \subset X\) is compact, then \(A\) is closed.
...
**9.** Define a continuous function \(f:C \to [0,1]\) which is onto where \(C\) is the Cantor set and \([0,1]\) is the unit interval.
...
**14.** What does it mean for a topological space \(X\) to be Hausdorff? Regular? Normal?
