...
**2.** Prove that \(\pi_1(S^1,1)\cong\mathbb{Z}\).
...
**7.** Show that if \(h,h':X\to Y\) are homotopic and \(k,k':Y\to Z\) are homotopic, then \(k\circ h\) and \(k'\circ h'\) are homotopic.

**8.** Let \(E\) and \(B\) be topological spaces. Define what it means for a function \(p:E\to B\) to be a covering map.

**9.** If \(j:A\hookrightarrow X\) is the inclusion of a retract, then show that the induced map on fundamental groups is injective.
...
**15.** Give an example of a topological space whose fundamental group is \(\mathbb{Z}/3\mathbb{Z}\). Give an example of a topological space whose fundamental group is \(\mathbb{Z}/2\mathbb{Z}\times\mathbb{Z}/2\mathbb{Z}\).
