...
* (a) Let \(f,g:X\to Y\) be continuous maps. Define what it means for \(f\) to be homotopic to \(g\).
* (b) Suppose that the continuous maps \(f,g:X\to Y\) are homotopic and that the continuous maps \(h,k:Y\to Z\) are homotopic. Show that \(h\circ f\) is homotopic to \(k\circ g\).
...
* (d) Does \(\mathbb{RP}^2\) have a cover, \(p:E\to\mathbb{RP}^2\), whose fundamental group, \(\pi_1(E)\), is \(\mathbb{Z}/(3)\)? Explain why or why not.
* (e) Does \(\mathbb{RP}^2\) have a cover, \(p':E'\to\mathbb{RP}^2\), whose covering transformation group, \(G(E')\), is \(\mathbb{Z}/(5)\)? Explain why or why not.
...
* (a) Define what it means for \(f:(C,d)\to(C',d')\) to be a morphism of chain complexes, also called a chain map.
* (b) For \(n\in\mathbb{Z}\) define the induced map \(H_n(f):H_n(C,d)\to H_n(C',d')\).
* (c) Prove that \(H_n(f)\) is well defined.

**8.** For \(n\geq 0\), let \(\Delta^n=\{(x_0,\ldots,x_n)\in\mathbb{R}^{n+1}\mid x_i\geq 0,\ \sum_i x_i=1\}\).
