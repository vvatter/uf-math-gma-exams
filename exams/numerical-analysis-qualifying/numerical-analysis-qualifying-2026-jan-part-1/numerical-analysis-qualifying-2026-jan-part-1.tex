...
\hypersetup{
  pdftitle={Numerical Analysis Qualifying Exam, January 2026, Part 1},
...
\renewcommand{\@maketitle}{%
  ...
  \vskip -0.5em%
  \tagstructbegin{tag=H1,title={Numerical Analysis Qualifying Exam, January 2026, Part 1}}%
  \tagpdfparaOff%
  \tagmcbegin{tag=H1}%
    {\LARGE\bfseries\href{https://gma.math.ufl.edu/exams/numerical-analysis-qualifying/}{Numerical Analysis Qualifying Exam}, January 2026, Part 1\par}%
  \tagmcend%
  \tagpdfparaOn%
  \tagstructend%
  \vskip 0.25em%
...
\makeatother
\title{Numerical Analysis Qualifying Exam, January 2026, Part 1}
\author{}
\date{}
\begin{document}
\maketitle
...
\item[\textnormal{(b)}]
If \(n\times n\) matrix~\(A\) is diagonal, then for any matrix~\(p\)-norm \(\|\cdot\|_p\), it holds that \(\|A\|_p=\max_{1\le i\le n}|a_{ii}|\).
\end{examsubparts}
