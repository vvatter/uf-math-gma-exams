...
\hypersetup{
  pdftitle={Numerical Linear Algebra First-Year Exam, January 2024},
...
\renewcommand{\@maketitle}{%
  ...
  \vskip -0.5em%
  \tagstructbegin{tag=H1,title={Numerical Linear Algebra First-Year Exam, January 2024}}%
  \tagpdfparaOff%
  \tagmcbegin{tag=H1}%
    {\LARGE\bfseries\href{https://gma.math.ufl.edu/exams/numerical-linear-algebra-first-year/}{Numerical Linear Algebra First-Year Exam}, January 2024\par}%
  \tagmcend%
  \tagpdfparaOn%
  \tagstructend%
  \vskip 0.25em%
...
\makeatother
\title{Numerical Linear Algebra First-Year Exam, January 2024}
\author{}
\date{}
\begin{document}
\maketitle
...
\item
Let \(A\in\mathbb{C}^{m\times n}\), with \(m\ge n\) and \(\operatorname{rank}(A)=p=n\ge 3\). Let \(a_1,a_2,\ldots\) denote the columns of~\(A\).
\begin{examsubparts}
\item[\textnormal{(a)}]
